...
if ( CALLED_FROM_SLACK_COMMAND && ! isset( $_POST['_mode'] ) ) {

	// Re-execute the file that this command was called on, so we have time to complete our searches
	// We'll then respond to the user via the slash command response_url

	//echo 'Loading the next ' . $train_count . ' ' . $stop_mode_plural . ' for you, standby...';

	$_POST['_mode'] = 'use_response_url';
	$ch = curl_init();
...
foreach ( $_trains as $train ) {
	...
	if (
		0 === $stop_mode &&
		( ! CALLED_FROM_SLACK_COMMAND || isset( $_POST['_mode'] ) ) &&
	...
	$trains[] = array (
		'direction' => $direction,
		'direction_id' => $train->platform->direction->direction_id,
		'line_name' => (
			0 === $stop_mode ?
...
if ( count( $trains ) ) {
	...
	foreach ( $trains as $key => $train ) {
		...
		if ( $train['short_run'] ) {
			if ( ( CALLED_FROM_SLACK_COMMAND && ! isset( $_POST['_mode'] ) ) || DISABLE_CONNECTING_TRAINS ) {
		...
		if ( ! CONDENSED_MODE && ! count( $payload['attachments'] ) ) {
			...
			$fallback = '' . 
				'The next ' . ( isset( $one_direction ) ? $train['direction'] . ' ' : '' ) .
				$stop_mode_singular . ' leaves ' . get_short_name( $stop_name ) . ' ' . $fallback_time .
				( 0 !== $stop_mode ? ' (' . $train['line_number'] . ')' : '' ) . '. ' .
				'Open Slack for more.';
		}
	...
	if ( $short_run_included ) {
		if (
			( ( CALLED_FROM_SLACK_COMMAND && ! isset( $_POST['_mode'] ) ) || DISABLE_CONNECTING_TRAINS ) &&
			! CONDENSED_MODE
		) {
			$payload['attachments'][] = array (
				'pretext' => "\n" .
